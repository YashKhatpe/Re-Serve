        ...
        <InfoWindow position={position} onCloseClick={() => setIsOpen(false)}>
          <div className="donation-popup p-3 text-black max-w-xs">
            <h3 className="text-lg font-bold mb-2">
              {donations.length > 1
                ? `${donations.length} Food Items Available`
                : donations[0].food_name}
            </h3>
            ...
            <div className="max-h-80 pr-1">
              {donations.map((donation) => (
                <div
                  key={donation.id}
                  className="mb-4 pb-3 border-b border-gray-200 last:border-0 last:mb-0 last:pb-0"
                >
